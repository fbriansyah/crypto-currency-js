...
describe('Block', () => {
  ...
  const nonce = 1;
  const difficulty = 1;
  const block = new Block({data, hash, lastHash, timestamp, nonce, difficulty});
  ...
  describe('mineBlock()', () => {
    ...
    it('sets the `lastHash` tobe the `hash` of lastBlock', () => {
    ...
    it('sets a `hash` that matches the difficulty criteria', () => {
      expect(
        hextToBinary(minedBlock.hash).substring(0, minedBlock.difficulty)
      ).toEqual(
